...
const ProjectStatus = (props) =>{ //project状态确认模块
    ...
    dataIndex: 'avatar',
    width: 48,
    render:(text,record)=>(<span style={{display:'inline-block'}}>
        <img alt="avatar" width={48} src={record.avatar} style={{borderRadius:"50%"}} />
    </span>)
    }, {
    title: 'content',
    dataIndex: 'content',
    ...
    return(
        <Tabs type="card"  tabBarStyle={{height:'54px',margin:'0px'}}>
            <TabPane tab="Tasks Completed" key="1">
                {
                    dataSource.length === 2 ? <Table 
                        pagination={false}  
                        showHeader={false} 
                        rowKey={(record, key) => key} 
                        dataSource={dataSource[0].pjtCompleted} 
                        columns={columns}>
                    </Table>:null
                }
            </TabPane>
            <TabPane tab="Tasks On Plan" key="2">
                {
                    dataSource.length === 2? <Table 
                        ...
                        showHeader={false} 
                        rowKey={(record, key) => key} 
                        dataSource={dataSource[1].pjtOnPlan} 
                        columns={columns}>
                    </Table>:null
                }
            </TabPane>
